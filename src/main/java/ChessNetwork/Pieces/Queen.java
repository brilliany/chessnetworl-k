...
public class Queen {
    ...
    public static boolean isValidMove(Move move, int[][][] chessboard, int x, int y, int color) {
        ...
        //check that move is either horizontal, vertical or diagonal
        if (move.getToX() != x && move.getToY() != y && Math.abs(move.getToX() - x) != Math.abs(move.getToY() - y)) {
            return false;
        }
